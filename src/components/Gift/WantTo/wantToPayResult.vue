  ...
  <div>
    <div v-if="getOrder" class="panel-padding">
      ...
      <card>
        <div slot="content">
          <group>
            <cell title="订单金额" :value="orderFee(getOrder)"></cell>
            <cell title="订单编号" :value="orderNo(getOrder)"></cell>
          </group>
        </div>
      </card>

      <div v-if="getOrder.orderType === 1">
        <div class="gain-other">
          <div class="gain-panel text-align-center">
            <router-link v-if="getOrder.orderType === 1" :to="toAddressList()">
